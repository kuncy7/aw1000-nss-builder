...
for i in $SEC; do
	...
	if [ -z "$IP" ]; then
		S6=$i
		[ -n "$(ubus list network.interface."$i"_6 2>/dev/null)" ] && S6=$i"_6"
		IP=$(ubus call network.interface status '{"interface":"'$S6'"}' 2>/dev/null | jsonfilter -q -e "@['ipv6-address'][0].address")
	fi
	printf " | %-""$LINE""s |\n" "$i: $PROTO, ${IP:-"?"}"
done
